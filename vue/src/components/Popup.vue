        ...
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-3">
                  ...
                  <button @click="savePayment()" class="text-lg rounded p-2 bg-gray-800 text-blue-400">Add!</button>
        ...
        <tbody class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
              <tr>
				<td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
					...
					required>
						<option disabled selected>Choose a Doc</option>
						<option value="Advanced">Advanced</option>
						<option value="Late Pay">Late Pay</option>
						<option value="Exchange">Exchange</option>
						<option value="Exchange">Refund</option>
					</select>
                </td>
                ...
                <td class="px-6 py-4">
					...
					class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 pl-2 pr-0 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
					required>
						<option disabled selected>What Currency</option>
...
export default {
	props: ['TogglePopup', 'creditor_id', 'remaining'],
	mounted:function(){

	document.getElementById('PERCENTAGE').placeholder = `max's ${this.remaining}`
	document.getElementById('PERCENTAGE').max = `${this.remaining}`
	...
	methods:{
		savePayment(){
			var mythis = this;
			this.model.payment.creditor_id = this.creditor_id;
			console.log(this.model.payment)
			axios.post("http://localhost:8000/api/payments", this.model.payment)
			.then(res => {
				// alert(res.data);
